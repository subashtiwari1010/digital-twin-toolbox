...
def archive_contains_mesh(zip_filenames: list[str]) -> bool:
    for zip_filename in zip_filenames:
        if Path(zip_filename).suffix.lower() == ".obj":
            return True
    return False
...
def resolve_mesh_size(asset_id: str, extension: str, upload_result: dict | None) -> list[float] | None:
    ...
    if input_file.lower().endswith(".obj"):
        return estimate_mesh_size_from_obj(input_file)
    return None
