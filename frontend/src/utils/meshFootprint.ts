...
export function computeMeshFootprintBbox(
  latitude: number,
  longitude: number,
  meshSizeMeters: number[],
): MapBbox | null {
  const size = parseMeshSize(meshSizeMeters)
  if (!size) {
    return null
  }
  const [widthMeters, depthMeters] = size
  return computeMeshFootprintBboxFromDimensions(
    latitude,
    longitude,
    widthMeters,
    depthMeters,
...
export function formatBboxCoord(value: number): string {
